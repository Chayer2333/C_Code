    ...
    do {
        ...
        switch(choice) {
            ...
            case 1: {			// 录入操作 
				...
				printf("\n请按如下格式录入学生信息：学号,姓名,性别(M/F),年龄,大学语文,高等数学,英语\n");
		        do{
		            if(studCount >= maxStuds) {
		                printf("\n学生信息数量已达到上限 %d，无法添加更多学生！\n", maxStuds);
		                break;
		            }
		            printf("\n请输入第 %d 个学生的信息: ", studCount+1);
...
void input(Student *stud) {
	...
	while (maxAttempts-- > 0) {		// 防止 upDate_one 调用太多导致栈溢出或死循环
		...
		errorField = 0;  			// 重置错误字段
	    int validationResult = validate(stud, VALIDATE_ALL, &errorField);
...
void upDatesys() {
	...
	for(i=0;i<studCount;i++) {
		if(nStud[i].id==id) {
			...
			printf("\n找到学生信息：\n");
			print(&nStud[i]);
			printf("\n信息指标：学号->1 ,姓名->2 ,性别(M/F)->3 ,年龄->4 ,大学语文->5 ,高等数学->6 ,英语->7 \n");
			printf("请输入：");
			scanf("%d",&choice);
	...
	switch(choice) {
		...
		case 3: {
			while(1) {
                ...
                if (!validate(stud, validateFlags, &errorField)) return;
                disError(errorField, stud);
            }
		}
		break;
